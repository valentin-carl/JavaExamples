...
public class XiangqiGame extends Game implements Serializable {
	...
	public boolean tryMove(String moveString, Player player) {
		...
		// is it player's turn?
		if (this.isRedNext() != redPlayer.equals(player)) {
			System.out.println("XiangqiGame :: tryMove | it is not the player's turn");
			return false;
		}
		
		// is move possible?
		if (!this.board.isMovePossible(moveString, this.isRedNext() ? Colour.RED : Colour.BLACK)) {
			System.out.println("XiangqiGame :: tryMove | move is not possible");
			return false;
		}
		
		// doMove
		this.board.doMove(moveString);
		
		// Add move to game history
		Move move = new Move(moveString, this.board.getFen(), player);
		this.history.add(move);
		
		// set next player
		this.nextPlayer = this.nextPlayer == redPlayer ? blackPlayer : redPlayer;
		
		// check for game over 
		if (!this.board.hasAnyMovesLeft(this.nextPlayer == redPlayer ? Colour.RED : Colour.BLACK)) {
			this.regularGameEnd(this.nextPlayer == redPlayer ? blackPlayer : redPlayer);
		}
